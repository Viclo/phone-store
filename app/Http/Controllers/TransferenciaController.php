...
class TransferenciaController extends Controller
{
    public function index(Request $request) 
    {
        ...
        if ($buscar==''){ 
            ...
            'transferencias.estado','users.usuario','users.idrol')
            ->where('transferencias.estado','!=','Eliminada')
            //->where('transferencias.origen',$oveja)
            ->Where('transferencias.destino',$oveja)
            ->orderBy('transferencias.id', 'desc')->paginate(10);
        }
        else{
            if($criterio=='codigo'){
                $transferencias = Transferencia::join('users','transferencias.idusuario','=','users.id')
                ->join('detalle_transferencias','transferencias.id','=','detalle_transferencias.idtransferencia')
                ->join('articulos','detalle_transferencias.idarticulo','=','articulos.id')
                ->select('transferencias.id','transferencias.tipo_comprobante',
                'transferencias.num_comprobante','transferencias.fecha_hora','transferencias.Receptor','transferencias.origen','transferencias.destino','transferencias.total','articulos.codigo as imei',
                'transferencias.estado','users.usuario')
                ->where('articulos.'.$criterio, 'like', '%'. $buscar . '%')
                //->where('transferencias.origen',$oveja)
                //->orWhere('transferencias.destino',$oveja)
                ->orderBy('transferencias.id', 'desc')->paginate(10);
                }
                else{
                $transferencias = Transferencia::join('users','transferencias.idusuario','=','users.id')
                ...
                'transferencias.num_comprobante','transferencias.fecha_hora','transferencias.Receptor','transferencias.origen','transferencias.destino','transferencias.total',
                'transferencias.estado','users.usuario')
                ->where('transferencias.'.$criterio, 'like', '%'. $buscar . '%')
                //->where('transferencias.origen',$oveja)
                //->orWhere('transferencias.destino',$oveja)
                ->orderBy('transferencias.id', 'desc')->paginate(10);
            }
    ...
    public function store(Request $request)
    {
        ...
        try{
            ...
            $transferencia->fecha_hora = $request->fechat;
    ...
    public function registro(Request $request){
        ...
        for($i=0;$i<count($articulos->toArray());$i++){
            $arti=$articulos[$i];
        ...
        $articulo->fechaA = $request->fechaA;
        ...
        $articulo->save();  
        $detalleTransferencia= (DetalleTransferencia::where('idtransferencia',$request->idc)->where('idarticulo',$arti->id)->get())[0];

            $detalleTransferencia->idarticulova = $articulo->id;
            $detalleTransferencia->update(); 
