...
class IngresoController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($buscar==''){
            ...
            if ($criterio=='codigo'){
                ...
                ->select('ingresos.id','ingresos.tipo_comprobante','ingresos.serie_comprobante',
                'ingresos.num_comprobante','ingresos.fecha_hora','ingresos.impuesto','ingresos.total','ingresos.moneda','articulos.codigo as imei',
                'ingresos.estado','personas.nombre','users.usuario')
                ->where('articulos.'.$criterio, 'like', '%'. $buscar . '%')
                ->where('ingresos.estado','!=','Anulado')
                ->orderBy('ingresos.num_comprobante', 'desc')->paginate(20);
                }
                else if($criterio=='nombre'){
                    $ingresos = Ingreso::join('personas','ingresos.idproveedor','=','personas.id')
                    ->join('users','ingresos.idusuario','=','users.id')
                    ->select('ingresos.id','ingresos.tipo_comprobante','ingresos.serie_comprobante',
                    'ingresos.num_comprobante','ingresos.fecha_hora','ingresos.impuesto','ingresos.total','ingresos.moneda',
                    'ingresos.estado','personas.nombre','users.usuario')
    ...
    public function update(Request $request) 
    {
        ...
        $ingreso->pago2 = $request->pago2;
        $ingreso->fecha_2 = $request->fecha2;
        if($request->pago3 != 0){
            $ingreso->pago3 = $request->pago3;
            $ingreso->fecha_3 = $request->fecha3;
            }
