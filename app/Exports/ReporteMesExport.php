...
class ReporteMesExport implements FromView
{
    /**
    * @return \Illuminate\Support\Collection
    */
    use Exportable;
    public function porMes(int $mes, int $anio, int $sucu)
    {
        $this->mes = $mes;
        $this->anio = $anio;
        $this->sucu = $sucu;
    ...
    public function view(): View
        {
            return view('pdf.ventasDetMes', [
                ...
                ->whereMonth('ventas.fecha_hora', $this->mes)
                ->whereYear('ventas.fecha_hora', $this->anio)
                ->orderBy('ventas.num_comprobante', 'desc')->get()
            ],['restas' => Venta::join('detalle_ventas','ventas.id','=','detalle_ventas.idventa')
            ->join('articulos','detalle_ventas.idarticulo','=','articulos.id')
